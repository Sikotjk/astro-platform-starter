...
describe('Auth & KYC-Gate', () => {
  it('registriert Sender und Traveler', async () => {
    const s = await request(http()).post('/auth/register').send({
      email: 'sender@e2e.de', password: 'password123', firstName: 'Sender', lastName: 'Test', role: 'SENDER',
    });
    ...
    const t = await request(http()).post('/auth/register').send({
      email: 'traveler@e2e.de', password: 'password123', firstName: 'Traveler', lastName: 'Test', role: 'TRAVELER',
    });
  ...
  it('lehnt doppelte Registrierung ab (409)', async () => {
    const r = await request(http()).post('/auth/register').send({
      email: 'sender@e2e.de', password: 'password123', firstName: 'X', lastName: 'Y',
    });
  ...
  it('blockt Trip-Erstellung ohne KYC (403)', async () => {
    const r = await request(http()).post('/trips').set(auth(state.travelerToken)).send({
      originAirport: 'FRA', destinationAirport: 'DYU', departureAt: '2026-09-01T10:00:00Z', capacityKgTotal: 10, pricePerKg: 8,
    });
...
describe('Zoll-Compliance', () => {
  it('Vorschau: blockt Bargeld (BLOCK, declarable=false)', async () => {
    const r = await request(http()).post('/customs/evaluate').send({
      items: [{ category: 'OTHER', description: 'Umschlag mit Bargeld', quantity: 1, unitValueEur: 500, isSealed: false }],
    });
    expect(r.status).toBe(201);
  ...
  it('lehnt Paket mit verbotenem Inhalt ab (422)', async () => {
    const r = await request(http()).post('/packages').set(auth(state.senderToken)).send({
      title: 'Verboten', weightKg: 1, declaredValueEur: 10, recipientName: 'R', recipientPhone: '+992', recipientCity: 'DYU',
      items: [{ category: 'OTHER', description: 'Waffe', quantity: 1, unitValueEur: 0, isSealed: false }],
    });
...
describe('Kompletter Buchungs-Lebenszyklus', () => {
  it('verifizierter Traveler kann einen Trip anlegen', async () => {
    ...
    await prisma.user.update({
      ...
      data: { kycStatus: 'VERIFIED', kycVerifiedAt: new Date(), stripeAccountId: 'acct_e2e', payoutsEnabled: true },
    });
    await prisma.user.update({ where: { id: state.senderId }, data: { stripeCustomerId: 'cus_e2e' } });

    const r = await request(http()).post('/trips').set(auth(state.travelerToken)).send({
      originAirport: 'fra', destinationAirport: 'dyu', departureAt: '2026-09-01T10:00:00Z',
      capacityKgTotal: 15, pricePerKg: 8, acceptedCategories: ['CLOTHING', 'GIFTS'],
    });
    expect(r.status).toBe(201);
  ...
  it('Sender legt Paket an (Zoll ALLOW)', async () => {
    const r = await request(http()).post('/packages').set(auth(state.senderToken)).send({
      title: 'Geschenke', weightKg: 3, declaredValueEur: 90, recipientName: 'Firuza', recipientPhone: '+992', recipientCity: 'Dushanbe',
      items: [{ category: 'CLOTHING', description: 'Jacke neu', quantity: 1, unitValueEur: 60, isSealed: false }],
    });
    expect(r.status).toBe(201);
  ...
  it('Buchung mit korrektem Preis-Snapshot (item+fee)', async () => {
    const r = await request(http()).post('/bookings').set(auth(state.senderToken)).send({
      tripId: state.tripId, packageId: state.packageId, agreedWeightKg: 3,
    });
    expect(r.status).toBe(201);
  ...
  it('Compliance-Gate: Übergabe vor Bestätigung scheitert (409)', async () => {
    await request(http()).post(`/bookings/${state.bookingId}/accept`).set(auth(state.travelerToken)).expect(201);
    await request(http()).post(`/bookings/${state.bookingId}/escrow`).set(auth(state.senderToken)).expect(201);
    // PAID simulieren (Webhook-Effekt)
    await prisma.booking.update({ where: { id: state.bookingId }, data: { status: 'PAID', paymentStatus: 'ESCROW_HELD' } });

    const r = await request(http()).post(`/bookings/${state.bookingId}/handover`).set(auth(state.senderToken));
  ...
  it('durchläuft HANDED_OVER..CONFIRMED und zahlt nur itemPrice aus', async () => {
    await request(http()).post(`/bookings/${state.bookingId}/accept-terms`).set(auth(state.travelerToken)).expect(201);
    await request(http()).post(`/bookings/${state.bookingId}/handover`).set(auth(state.senderToken)).expect(201);
    await request(http()).post(`/bookings/${state.bookingId}/transit`).set(auth(state.travelerToken)).expect(201);
    await request(http()).post(`/bookings/${state.bookingId}/delivered`).set(auth(state.travelerToken)).expect(201);

    const r = await request(http()).post(`/bookings/${state.bookingId}/confirm`).set(auth(state.senderToken));
    expect(r.status).toBe(201);
  ...
  it('schreibt ein lückenloses Audit-Log', async () => {
    const r = await request(http()).get(`/bookings/${state.bookingId}`).set(auth(state.senderToken));
...
describe('Chat', () => {
  it('Teilnehmer senden & lesen', async () => {
    await request(http()).post(`/bookings/${state.bookingId}/messages`).set(auth(state.senderToken)).send({ body: 'Hallo' }).expect(201);
    await request(http()).post(`/bookings/${state.bookingId}/messages`).set(auth(state.travelerToken)).send({ body: 'Servus' }).expect(201);
    const r = await request(http()).get(`/bookings/${state.bookingId}/messages`).set(auth(state.senderToken));
    expect(r.body.length).toBe(2);
  });

  it('Aussenstehender wird abgewiesen (403)', async () => {
    const outsider = await request(http()).post('/auth/register').send({
      email: 'outsider@e2e.de', password: 'password123', firstName: 'Out', lastName: 'Sider',
    });
    const r = await request(http()).get(`/bookings/${state.bookingId}/messages`).set(auth(outsider.body.accessToken));
    expect(r.status).toBe(403);
  });
});
...
describe('Reviews', () => {
  it('Sender bewertet Traveler nach Abschluss', async () => {
    const r = await request(http()).post(`/bookings/${state.bookingId}/review`).set(auth(state.senderToken)).send({ rating: 5, comment: 'Top' });
    expect(r.status).toBe(201);
    expect(r.body.rating).toBe(5);
  });

  it('verhindert doppelte Bewertung (409)', async () => {
    const r = await request(http()).post(`/bookings/${state.bookingId}/review`).set(auth(state.senderToken)).send({ rating: 1 });
    expect(r.status).toBe(409);
  });
